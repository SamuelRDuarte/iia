...
class BayesNet:
    ...
    def individualProb(self, var, val):
        #all_conjunctions = [c for c in self._gen_conjunction(list(self.dependencies.keys())) if (var, val) in c]
        all_conjunctions = [list(zip(self.dependencies.keys(), list(line))) for line in product([True, False], repeat=len(self.dependencies.keys()))]
        conjunctions = [c for c in all_conjunctions if (var,val) in c]
        return sum([self.jointProb(c) for c in conjunctions])

    def _gen_conjunction(self, variables):
        if len(variables) == 1:
            return [[(variables[0], True)], [(variables[0], False)]]

        l = []
        for c in self._gen_conjunction(variables[1:]):
